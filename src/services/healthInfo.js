...
export const getTopicsByUserInfo = async (request) => {
  ...
  try {
    ...
    if (request.pregnant === true) {
      const pregnantResults = await axios.get('/api/topicsearch.json?categoryId=127')
      result.push(...pregnantResults.data.Result.Resources.All.Resource)
    }
    ...
    const result = response.data.Result.Resources.All.Resource
    console.log(result)
    const subCategories = getSubCategoriesToFilter(result)
    return { result: result.sort((a,b) => {return compareStrings(a.Categories, b.Categories)}).map(replaceCategoryString), subCategories }
  }
...
export const getTopicById = async (id) => {
  try {
    const response = await axios.get(`/api/topicsearch.json?topicId=${id}`)
...
export const getCategoryData = async () => {
  const response = await axios.get('/api/itemlist.json?lang=en&type=category')
